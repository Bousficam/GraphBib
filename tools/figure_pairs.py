...
def printed_page_offset(fm: dict, max_pdf_page: int, offline_ok: bool = True):
    """First printed page of the article, when it can be established.

    A marker file name gives the PDF page, not the printed page: an
    article that runs pages 111-118 of a journal has its Figure 1 on PDF
    page 4 and on printed page 114. Crossref knows the range, so the
    offset is a lookup, not a guess.

    Returns the first printed page, or None. Refuses when the range does
    not cover the pages the images claim (an offprint bundled with a
    supplement, an article-number journal with no range at all), because
    a wrong offset is worse than no page.
    """
    doi = normalize_doi(str(fm.get("doi") or ""))
    if not doi or not offline_ok:
        return None
    try:
        md = crossref_metadata(doi)
    except Exception:
        return None
    rng = (md or {}).get("pages") or ""
    m = re.match(r"^\s*(\d+)\s*-\s*(\d+)\s*$", rng)
    if not m:
        return None
    first, last = int(m.group(1)), int(m.group(2))
    if last < first or (last - first + 1) < max_pdf_page:
        return None
    return first
...
def collect(slug: str, raw_override: str = None) -> dict:
    ...
    for f in files:
        ...
        if m:
            max_idx = max(max_idx, int(m.group(1)) + 1)
    first_printed = printed_page_offset(fm_page or {}, max_idx) if max_idx else None
    ...
    for f in files:
        ...
        if printed_page is None and pdf_page is not None and first_printed is not None:
            printed_page = first_printed + pdf_page - 1
...
def main():
    ...
    print(f"  page anchors: {'found' if data['page_map'] else 'absent (pages from file names or unknown)'}")
    print()
    if not data["figures"]:
        print("  no images.")
        return 1
