...
This workflow (PRISMA 2020 screening + SR data extraction) has moved to
the sibling repo [TallyBib](../TallyBib/), split out so it can be run
and shared independently of the wiki agent. It forks (does not link to)
GraphBib's PDF -> Markdown conversion pipeline and HTTP helpers, so the
two repos can diverge freely from here.
